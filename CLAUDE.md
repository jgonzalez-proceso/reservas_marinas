...
Y una cita puede estar viva, en https y en host oficial, y aun así **no llevar a ninguna parte**. El IDEIB apunta dos normas contra su propia ficha del espacio y las dos dan **404**: el Decret 8/2023 (PORN de Llevant) y la Llei 2/2017 (es Trenc), que son precisamente las que prohíben la pesca submarina en esos dos parques. `SUSTITUCIONES`, en `normalize.js`, las redirige al texto consolidado del Institut d'Estudis Autonòmics — que no es un destino inventado: es el que `fuentes.js` ya citaba para esas mismas dos normas, así que la web venía diciendo la misma norma con dos direcciones distintas en la misma pantalla, una viva y otra muerta, según se mirara la ficha o la tarjeta de la figura. Y es mejor cita, porque el consolidado incorpora las modificaciones posteriores y el PDF del día de publicación no.

**La clave es la URL exacta publicada, para que la sustitución se desactive sola.** El día que el IDEIB arregle la suya dejará de casar y pasará la suya, sin que nadie tenga que acordarse de venir a borrar nada. Ahí es también donde acaba: una URL que responde se deja en paz aunque se conozca un destino mejor, porque redirigir lo que funciona es empezar a mantener un directorio de enlaces en lugar de arreglar citas rotas.

**Y solo se aplica al pintar, nunca al generar**, al revés que el ascenso a https. Un ascenso es la misma URL; una sustitución es otra distinta, y `normasDesdeAtributos` construye `normas[]` a partir de un `normaUrl` que ya ha pasado por `limpiaUrl`. Si la sustitución entrara ahí quedaría escrita en el geojson al siguiente `npm run data`, y el dato generado dejaría de decir qué publica el IDEIB. Con eso se pierden a la vez las dos cosas del párrafo anterior: la posibilidad de detectar que el IDEIB ha corregido su enlace —lo único que apaga esta tabla— y la comprobación de `rules:check`, que busca la clave en la cartografía publicada y la encontraría ya sustituida. Un dato generado registra lo que sirve la fuente; repararlo es cosa del sink.

Eso deja una cadena exacta sosteniendo dos citas, y una cadena exacta se rompe con una errata sin que falle nada: la sustitución dejaría de aplicarse en silencio. Por eso `rules:check` comprueba que cada clave **sigue apareciendo** en la cartografía publicada y que cada destino pasa la política de `urlOficial()`. No necesita red, así que va dentro del build. Si una clave deja de aparecer, el gate falla diciendo que o hay una errata o el IDEIB ha corregido su enlace y la entrada sobra — no es algo que se arregle relajando el control, igual que con el umbral de las coordenadas. Y `links:check` comprueba el **destino sustituido**, no el que ya no se sirve: si no, esas dos citas saldrían rojas para siempre en un informe que existe para que el rojo signifique algo.
...
**El bloque «Qué es este mapa» existe para que haya algo que indexar.** Sin él, un rastreador que no ejecute JavaScript veía **11 palabras**: el título, los botones y el aviso legal. Todo el trabajo del proyecto —las seis capas, las 96 zonas, la cita normativa por ficha— vivía en el bundle y en los GeoJSON, donde ningún buscador lo lee. Con él son 239. Va en el HTML estático y plegado por defecto, como la leyenda: el contenido es el mismo lo vea quien lo vea.

Va **en `.zona-pie`, hermano de `<footer>` y no hijo suyo**. Hermano porque el footer es información de cierre y esto es contenido principal —además, los extractores de contenido descartan `<footer>` entero por heurística, y ahí el recuento vuelve a 11—. En el mismo envoltorio porque es donde tiene que verse: abajo a la derecha, discreto, sin disputarle sitio al mapa.
