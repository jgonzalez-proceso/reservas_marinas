...
Está **dentro de `.mapa-envoltorio` y no sobre el pie**, y tampoco dentro de `<footer>`. Lo segundo porque el footer es para información de cierre y esto es contenido principal —además, los extractores de contenido descartan `<footer>` entero por heurística—. Lo primero porque **`--pie-alto` miente**: `.pie` declara 56 px, pero en pantallas estrechas el aviso legal envuelve a tres líneas y mide 116, así que cualquier posición calculada desde esa variable acaba tapada por el pie. Medido, no supuesto. Por eso el panel además acota su altura con un margen conservador y hace scroll: mientras el pie pueda crecer sin avisar, la única garantía es no apurar el hueco.
