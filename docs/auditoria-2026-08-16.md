...
- [x] **M2 · Precisión GPS desconocida se trata como precisión perfecta** — `src/engine/locate.js:203-208`. Hecho el 17/08/2026: con `accuracy` ausente, `NaN` o `Infinity` se asume un radio conservador de 50 m (`PRECISION_DESCONOCIDA_M`, exportada) en vez de `r = 0` — no saber la precisión es menos información que tener una mala, y el veredicto debe empeorar, no mejorar. El mensaje del panel lo declara: «Precisión GPS: desconocida (se asume ±50 m)», en las tres ramas del veredicto. Verificado con 11 casos (ausente/NaN/Infinity a 2 m → `dudosa`; a 60 m → veredicto firme; precisión conocida sin cambios; borde exacto; precisión negativa) y con el veredicto completo sobre un polígono sintético.

- [x] **M3 · `condicionesDeOtrasFiguras` incluye figuras MENOS restrictivas que la ganadora** — `src/engine/resolve.js:137-145`. Hecho el 17/08/2026: el filtro exige ahora, además de igual o menor área, **el mismo estado que la ganadora** — una figura menos restrictiva ha sido desplazada entera y sus condiciones describen cómo practicar la actividad bajo un régimen que ahí ya no manda. Como la ganadora es la de mayor rango y los rangos son únicos por estado, igual rango equivale a igual estado. Verificado con el escenario del hallazgo (dos figuras de geometría idéntica, una `prohibited` y otra `allowed_with_authorization` con «solo con caña y potera, de octubre a abril»: antes se colaba, ahora cero condiciones bajo la prohibición) y sin regresión sobre el punto real del Toro (las 5 condiciones de la zona especial de buceo, de igual estado que la ZAP, siguen llegando al panel).

- [x] **M4 · El `permit` NO sobrevive a la sustitución, contra lo que dicen CLAUDE.md y los comentarios** — `src/rules/schema.js:150-177` + `resolve.js:169`. Hecho el 17/08/2026: `resuelveHerencia` conserva ahora el permiso de la madre cuando la hija declara su propia regla sin permiso y su estado lo admite (`allowed_with_authorization` o `restricted`) — la zona interior endurece el cómo, no deroga la autorización del régimen general. Bajo `prohibited` no se hereda (no hay permiso que valga) y bajo `allowed` tampoco (declararlo afirma que no hace falta); un permiso propio nunca se pisa. `permitHeredado` marca la procedencia. Verificado con los cinco casos sintéticos; el barrido de las fichas reales confirma que hoy ninguna cambia —todas las hijas redeclaran o prohíben—, así que el arreglo alinea el código con el contrato documentado sin alterar ningún resultado actual. La validación sigue exigiendo permiso declarado en las `allowed_with_authorization`, de modo que la herencia cubre sobre todo el hueco real: las `restricted`.
